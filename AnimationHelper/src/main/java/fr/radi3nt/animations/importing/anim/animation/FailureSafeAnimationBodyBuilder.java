...
public class FailureSafeAnimationBodyBuilder implements AnimationBodyBuilder {
    ...
    public AnimationBody build(AnimFileContent content, AnimHeader header) {
        ...
        while (content.hasLine()) {
            ...
            animDataMap.put(new ChannelInfo(fullChannelType, mainChannelType, leafChannelType, objectName), animData);
        }
    ...
    private KeyframesData buildKeysData(AnimFileContent content, AnimHeader header) {
        ...
        while (content.hasLine()) {
            ...
            if (line.startsWith("}"))
                break;
            String[] splitLine = line.split(" ");
            int in = Integer.parseInt(splitLine[0]);
            float out = Float.parseFloat(splitLine[1]);
            ...
            keyData.add(new KeyData(in, out, inInterpolation, outInterpolation, tanLocked, weightLocked, curveHandleData));
        }
        content.nextLine();
